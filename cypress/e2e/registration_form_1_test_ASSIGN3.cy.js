...
describe('This is first test suite, Jaanus Hass', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        
        //This is the Assignment 3 made by Jaanus Hass; Date 20.11.2023
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="firstName"]').type('Jaanus')
        cy.get('input[name="lastName"]').type('Hass')
        cy.get('input[name="password"]').type('Assignment3_123')
        cy.get('[name="confirm"]').type('Assignment3_123')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        cy.get('#username').type('Jaanus123')
        cy.get("input[name='password']").type('Password123')
        cy.get('[name="confirm"]').type('Password123')
        cy.get('[data-testid="phoneNumberTestId"]').type('10203040').clear()
        cy.get('h2').contains('Password').click()
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get("input[name='password']").type('Password123')
        cy.get('[name="confirm"]').type('Password123')
        cy.get('[data-testid="phoneNumberTestId"]').type('10203040').clear().type('Cerebrum Hub')
        cy.get('h2').contains('Password').click()
        cy.get('.submit_button').should('not.be.enabled')
        cy.get('#success_message').should('not.be.visible')
